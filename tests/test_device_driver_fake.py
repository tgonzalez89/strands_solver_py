...
def test_fake_driver_execute_valid_path_changes_next_screenshot() -> None:
    ...
    driver = DeviceDriverFake(board=board, valid_moves=[valid_move], mode="light")

    before = driver.capture_screen()
    _, centers = render_board_png(board, mode="light")
...
def test_fake_driver_execute_invalid_path_keeps_screenshot_same() -> None:
    ...
    driver = DeviceDriverFake(board=board, valid_moves=[valid_move], mode="dark")

    before = driver.capture_screen()
    _, centers = render_board_png(board, mode="dark")
...
def test_fake_driver_capture_screen_reuses_cached_render(monkeypatch: pytest.MonkeyPatch) -> None:
    board = _board()
    driver = DeviceDriverFake(board=board, valid_moves=[[(0, 0), (0, 1), (0, 2), (0, 3)]], mode="light")
